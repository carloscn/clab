...
mod utils_fo {
    ...
    pub fn write_file_without_append(filename:&str, buf:&[u8]) -> Result<usize, io::Error>
    {
        if filename.is_empty() {
            return Err(io::Error::new(ErrorKind::InvalidInput, "filename is empty"));
        }
        ...
        let mut ctx = match OpenOptions::new()
            ...
            Ok(file) => file,
            Err(err) => {
                println!("the file open {:?}", err);
                return Err(err);
            },
        };

        let rc = ctx.write(buf)?;
        Ok(rc)
    }
